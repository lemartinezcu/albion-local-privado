-- Create Albion views (called at the project initialization AND when updating the data)
...
select row_number() over() as id, t.id as ${NAME}_id, sc.section_id, t.hole_id, t.from_, t.to_, sc.geom::geometry('LINESTRING', ${SRID}), ${T_FIELDS}
from _albion.${NAME} as t
...
create or replace function albion.${NAME}_section_instead_fct()
returns trigger
language plpgsql
as
$$$$
    ...
    begin
        ...
        if tg_op = 'INSERT' then
            insert into _albion.${NAME}(id, hole_id, from_, to_, ${FIELDS})
                values(new.id, new.hole_id, new.from_, new.to_, ${NEW_FIELDS})
                returning id into new.${NAME}_id;
            return new;
        elsif tg_op = 'UPDATE' then
            update _albion.${NAME} set id=new.${NAME}_id, hole_id=new.hole_id, from_=new.from_, to_=new.to_, ${SET_FIELDS}
            where id=old.${NAME}_id;
            return new;
        elsif tg_op = 'DELETE' then
            delete from _albion.${NAME} where id=old.${NAME}_id;
            return old;
        end if;
    end;
$$$$
...
create trigger ${NAME}_section_instead_trig
    instead of insert or update or delete on albion.${NAME}_section
       for each row execute procedure albion.${NAME}_section_instead_fct()
;
