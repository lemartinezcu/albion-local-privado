...
from qgis.PyQt.QtGui import QImage, QBrush, QColor, QPen, QWheelEvent
        ...
        if self.__redraw:
            with self.__project.connect() as con:
                ...
                cur.execute(
                    """SELECT geom, gamma
                    FROM albion.radiometry
                    WHERE hole_id='{}' AND gamma>=0""".format(
                        self.__id
                    )
                )
                ...
                cur.execute(
                    """ SELECT geom, rho
                        FROM albion.resistivity
                        WHERE hole_id='{}' AND rho>=0""".format(
                        self.__id
                    )
                )
                ...
                # mineralization
                cur.execute(
                    """ SELECT geom, oc, accu, grade
                        FROM albion.mineralization
                        WHERE hole_id='{}'""".format(
                        self.__id
                    )
                )
                ...
                for geom, oc, accu, grade in cur.fetchall():
                    ...
                    text.setPos(
                        tabs[5] + spacing,
                        -int(1.5 * fm.height()) + int(0.5 * (z_start + z_end)),
                    )
