CREATE OR REPLACE FUNCTION albion.elementary_volumes(cell_id_ character varying, graph_id_ character varying, geom_ geometry, holes_ character varying[], starts_ character varying[], ends_ character varying[], hole_ids_ character varying[], node_ids_ character varying[], nodes_ geometry[], end_ids_ character varying[], end_geoms_ geometry[], end_holes_ character varying[], end_node_relative_distance real, end_node_relative_thickness real)
        ...
        for p in polygons:
            ...
            for t in t_sfcgal.geoms:
                try:
                    ...
                    plpy.warning("Error inserting tri in elementary_volumes")
                    logging.info("Error inserting tri in elementary_volumes")
                    logging.info(e)
                    logging.info("tri value:")
                    logging.info(t.wktDecim())
                    logging.info("node_map value:")
...
for g, f1, f2, f3 in elementary_volumes(holes_, starts_, ends_, hole_ids_, node_ids_, nodes_, end_ids_, end_geoms_, end_holes_, 32632, end_node_relative_distance, end_node_relative_thickness):
    yield g, f1, f2, f3
$function$
