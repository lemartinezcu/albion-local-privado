...
def sym_split(l1, l2):
    if (l1[0][2] < l2[0][2] and l1[-1][2] > l2[-1][2]) or (
        l1[0][2] > l2[0][2] and l1[-1][2] < l2[-1][2]
    ):
...
def share_an_edge(t, f):
    return 2 == len(
        set([tuple(x) for x in t.exterior.coords[:3]]).intersection(
            set([tuple(x) for x in f.exterior.coords[:3]])
        )
    )
...
def face_edge_intersects(segment, crossing_segment):
    A, B, C, D = (
        array(segment[0]),
        array(segment[1]),
        array(crossing_segment[0]),
        array(crossing_segment[1]),
    )
...
def elementary_volumes(
    ...
    end_node_relative_thickness=0.3,
):
    ...
    nodes = {
        id_: wkb.loads(bytes.fromhex(geom)) for id_, geom in zip(node_ids_, nodes_)
    }
    ...
    for n in unused_nodes:
        ...
        if p:
            A, B, C = (
                array(nodes[n].coords[0][:2]),
                array(nodes[p[0]].coords[0][:2]),
                array(nodes[p[1]].coords[0][:2]),
            )
            c = average(array(nodes[n].coords), (0,))
            u = (
                0.5
                * (normalized(B - A) + normalized(C - A))
                * end_node_relative_distance
                * 0.5
                * (norm(B - A) + norm(C - A))
            )
            ...
            offsets[nodes[n].coords[0]] = tuple(
                c + array((u[0], u[1], +0.5 * end_node_thickness))
            )
            offsets[nodes[n].coords[-1]] = tuple(
                c + array((u[0], u[1], -0.5 * end_node_thickness))
            )
    ...
    for hl, hr, other_hole in (
        ...
        face_idx += 1
        direct_orientation = (
            (hl, hr) == (holes_[0], holes_[1])
            or (hl, hr) == (holes_[1], holes_[2])
            or (hl, hr) == (holes_[2], holes_[0])
        )

        face_edges = list(
            set(
                [
                    (s, e) if holes[s] == hl else (e, s)
        ...
        for i, j in combinations(list(range(len(face_lines))), 2):
            assert (
                face_lines[i].side != Line.VERTICAL
                and face_lines[j].side != Line.VERTICAL
            )
            p = sym_split(face_lines[i].points, face_lines[j].points)
            if p and p not in offsets:
                if (
                    face_lines[i].points[0] in offsets
                    and face_lines[i].points[-1] in offsets
                    and face_lines[j].points[0] in offsets
                    and face_lines[j].points[-1] in offsets
                ):
                    splt = sym_split(
                        offset_coords(
                        ...
                        offset_coords(
                            offsets, [face_lines[j].points[0], face_lines[j].points[-1]]
                        ),
                    )
            ...
            if p and p not in offsets:
                if (
                    ...
                    offsets[p] = tuple(
                        0.5
                        * (
                            array(offsets[face_lines[i].points[0]])
        ...
        for s, e in face_edges:
            alt = 0.5 * (nodes[s].coords[0][2] + nodes[e].coords[0][1])
            if alt > top_altitude:
        ...
        node_map = {
            (round(dot(p - origin, v), PRECI), round(dot(p - origin, z), PRECI)): p
            for e in linework
            for p in e
        }
        linework = [
            LineString(
                [
                    (
                        round(dot(e[0] - origin, v), PRECI),
                        round(dot(e[0] - origin, z), PRECI),
                    ),
                    (
                        round(dot(e[1] - origin, v), PRECI),
                        round(dot(e[1] - origin, z), PRECI),
                    ),
                ]
            )
            for e in linework
        ...
        if DEBUG:
            ...
            for i, li in enumerate(linework):
                ...
                for j, lj in enumerate(linework):
                    if i != j and (
                        not (
                            lj.coords[0] != li.coords[0] or lj.coords[1] != li.coords[1]
                        )
                        or not (
                            lj.coords[0] != li.coords[1] or lj.coords[1] != li.coords[0]
                        )
                    ):
                        open(
                            "/tmp/dup_line_{}_face_{}.vtk".format(bug, face_idx), "w"
                        ).write(
                            to_vtk(
                                MultiLineString([LineString(linework_sav[j])]).wkb_hex
                            )
                        )
                        # fix_print_with_import
                    ...
                    if (
                        i != j
                        and li.coords[1] == lj.coords[0]
                        or li.coords[1] == lj.coords[1]
                    ):
                        found = True
                if not found:
        ...
        domain = unary_union(
            [
                Polygon(
                    [
                        (
                            round(dot(p - origin, v), PRECI),
                            round(dot(p - origin, z), PRECI),
                        )
                        for p in array(dom.exterior.coords)
                    ]
                )
                for dom in domain
            ]
        ...
        for p in polygons:
            ...
            for t in tessellate(p):
                ...
                q = (
                    Polygon([node_map[tri[0]], node_map[tri[1]], node_map[tri[2]]])
                    if direct_orientation
                    else Polygon([node_map[tri[2]], node_map[tri[1]], node_map[tri[0]]])
                )
        ...
        for t in term_tri:
            ...
            for s in zip(t.exterior.coords[:-1], t.exterior.coords[1:]):
                if s in edges:
                    if (
                        (
                            is_segment(s, top_lines)
                            or is_segment(s, bottom_lines)
                            or s in end_lines
                        )
                        and s[0] in offsets
                        and s[1] in offsets
                    ...
                    if (s[1], s[0]) in end_lines:
                        ...
                        faces[
                            tuple(sorted((end_lines[(s[1], s[0])], other_hole)))
                        ] += terms[-2:]
        termination += terms

    if DEBUG:
        open("/tmp/faces.obj", "w").write(to_obj(MultiPolygon(result).wkb_hex))
        open("/tmp/termination.obj", "w").write(
            to_obj(MultiPolygon(termination).wkb_hex)
        )
    ...
    if len(result):
        ...
        if DEBUG:
            ...
            open("/tmp/linework_bottom_unm.vtk", "w").write(
                to_vtk(
                    MultiLineString([LineString(e) for e in bottom_linework]).wkb_hex
                )
            )
        ...
        for face, side in zip(("top", "bottom"), (top_linework, bottom_linework)):
            ...
            for m in merged:
                if has_proper_2d_topology(m):
                    ...
                    for t in tessellate(p):
                        ...
                        q = (
                            Polygon(
                                [node_map[tri[0]], node_map[tri[1]], node_map[tri[2]]]
                            )
                            if face == "bottom"
                            else Polygon(
            ...
            if DEBUG:
                open("/tmp/face_{}.obj".format(face), "w").write(
                    to_obj(MultiPolygon(face_triangles).wkb_hex)
                )
    ...
    for n, l in list(ends.items()):
        if len(l) == 2:
            ...
            l = l
            if dot(cross(B - A, C - A), array((0.0, 0.0, 1.0))) <= 0:
                l = list(reversed(l))
                k1, k2 = k2, k1
            termination += [
    ...
    for c in connected:
        ...
        face1 = translate(
            MultiPolygon(face1), translation[0], translation[1], translation[2]
        )
        geos.lgeos.GEOSSetSRID(face1._geom, srid_)

        face2 = translate(
            MultiPolygon(face2), translation[0], translation[1], translation[2]
        )
        geos.lgeos.GEOSSetSRID(face2._geom, srid_)

        face3 = translate(
            MultiPolygon(face3), translation[0], translation[1], translation[2]
        )
        geos.lgeos.GEOSSetSRID(face3._geom, srid_)

        empty_mp = "SRID={} ;MULTIPOLYGONZ EMPTY".format(srid_)
        yield (
            res.wkb_hex if not res.is_empty else empty_mp,
